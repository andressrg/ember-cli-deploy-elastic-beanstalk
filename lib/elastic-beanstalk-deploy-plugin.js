...
module.exports = DeployPlugin.extend({
  defaultConfig: {
    ...
    emberOutputPath: path.join('tmp', 'deploy-dist'),
  ...
  willUpload: function(context){
    ...
    var dir = this.readConfig('emberOutputPath');

    zipPath = path.resolve(zipPath);

    this.log('zipping ' + dir + ' into ' + zipPath, { verbose: true });

    return exec("zip -r " + zipPath + " deploy-dist", {
  ...
  upload: function(context) {
    ...
    var accessKeyId = this.readConfig('s3AccessKeyId');
    if (accessKeyId) {
      awsConfig.accessKeyId = accessKeyId;
    }
    var secretAccessKey = this.readConfig('s3SecretAccessKey');
    if (secretAccessKey) {
  ...
  didUpload: function(context) {
    ...
    var accessKeyId = this.readConfig('ebAccessKeyId');
    var secretAccessKey = this.readConfig('ebSecretAccessKey');
    var bucket = this.readConfig('bucket');
